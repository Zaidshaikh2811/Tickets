...
describe("Orders API", () => {
    ...
    it("creates multiple tickets and updates each twice", async () => {
        const createAndUpdateTicket = async (i: number) => {
            ...
            ticket.set({ price: ticket.price + 5 });
            await ticket.save();
        ...
        for (let i = 0; i < 400; i++) {
            await createAndUpdateTicket(i);
        }
        const allTickets = await Ticket.find();
        expect(allTickets.length).toBe(400);
        allTickets.forEach(t => {
            expect(t.price % 10).toBe(0);
        });
    });


    it("does not call if version number is skipped during updates", async () => {
        const ticket = Ticket.build({
            id: new mongoose.Types.ObjectId().toHexString(),
            title: "version-skip-ticket",
            price: 100,
            userId: new mongoose.Types.ObjectId().toHexString(),
            version: 0,
        });
        ...
        ticket.set({ version: 2 });

        await expect(ticket.save()).rejects.toThrow("No matching document found");
